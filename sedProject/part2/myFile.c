...
#include/**/<complex.h>BEEPBOOP//*Oh,my_dear_friend.How_I've_missed_you.--C-3PO***/
typedef/**/complexBEEPBOOP/double(c);c(X)[3],P,O;cBEEPBOOP/B(double t){double s=1-t,u;P=
...
void/**/b(    double   t,BEEPBOOP*   *   **/double   u){double   s=P=B(t)-B(u);(s=P
...
#include/*****    your   father..   --DarthVader   **/   <time.h>BEEPBOOP**   ****/
int(main)(int    (x),   char**V){;   clock_t(c)=   /*   */clock();;;   for(d=D
;m<26;m++,s    ++)*s>   63?*d++=m%   7*          16-7   *8,*d++=m/   7*25,*d++
=*s-64:0;;    ifS(V[1])   {;;;FILE   *F   =fopen(V[+1],   "r");for   (d=D,L=N=m
=0;(x=/**             *            ***              **/            fgetc(F))>0
||fclose(F);)ifS(x>13?64<x&&x<91?*d++=m*16,*d++=L*25,*d++=x%26:0,m++,0:1)for(++
...
=G;for(;n--;)for(;*p++>33;);*a=a[1]=*p++;for(;*p>33;p++)ifS(*p%2?*a=*p,0:1){a[2
